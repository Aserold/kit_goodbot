...
def parse_subs():
    ...
    group_list = [group.text for group in soup.find_all('td', class_='section') if group.text != '*']
    ...
    for row in soup.find_all('tr'):
        if row.find('td', class_='section'):
            ...
            pair_data = [cell.text.strip() for cell in row.find_all('td', class_='content')]
            if len(pair_data) >= 5:
                pair_info = {
    ...
    for item in schedule_data:
        if 'group' in item:
            item['lectures'] = [pair for pair in item['lectures'] if pair != default_pair_info]
